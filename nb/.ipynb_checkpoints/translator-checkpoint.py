...
def reg_cmp_behaviour(result_file_html, result_file_txt, command : str, ref_mask : list, ref_data : list, last_rd_data : list) -> None :
    ...
    if (nb_rd_bytes != len_ref_data): 
        ...
        error_msg.number_of_packets_mismatch(result_file_html, result_file_txt, len_ref_data, nb_rd_bytes)
        test_result = "error"
    else :
    ...
    if (len_ref_data != nb_rd_bytes): 
        ...
        for hexa_packet_cnt in range(len_ref_data):
            ...
            if not one_byte_test_result:
                test_result = False
    if (test_result):    
        valid_msg.valid_cmp(result_file_html, result_file_txt, data_ref, last_rd_data)
    else :
        error_msg.invalid_cmp(result_file_html, result_file_txt, data_ref, last_rd_data)
...
def equal_bytes(result_file_html, result_file_txt, ref_byte : int, rd_byte : int, storage : str) -> bool :
    ...
    if (ref_byte != rd_byte): # in case hex data differs
        ...
        if (storage == "reg"):
            ...
            error_msg.mem_cmp_match(result_file_html, result_file_txt, ref_hex_byte, rd_hex_byte)
        return True
